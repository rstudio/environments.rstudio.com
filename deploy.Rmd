...
Sound familiar? These steps are the same steps used in the ["snapshot and restore" strategy](./reproduce.html#snapshot-and-restore-using-renv) for reproducing environments. Take a look at the common challenges and questions, as the same concerns apply for production deployments.
...
In the context of [reproducibility strategies](./reproduce.html), it is possible for the development environment to use a [validated](./reproduce.html#validated) or [shared baseline](./reproduce.html#shared-baselines) strategy, while production deployments for data propducts use the [snapshot](./reproduce.html#snapshot-and-restore-using-renv) strategy.
