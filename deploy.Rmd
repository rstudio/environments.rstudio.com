...
These steps are the same steps used in the ["snapshot and restore" strategy](./snapshot.html) for reproducing environments. The following two sections provide example steps for implementing this process. These examples are not exhaustive, other processes can work, but they should meet the key requirements of record, restore, and isolate.
...
3. The RStudio Connect server restores the environment from the manifest, by matching the requested version of R to the available [versions of R installed](./r-installation.html), and then installing the required packages from the listed [repository](./repository.html).
...
In this example, a Docker container is used to isolate the data product, and  `renv` is used to restore the appropriate package environment. More details are available for using [R with Docker here](./docker.html).
...
In this example, the version of R is controlled by the base image, using an image provided by RStudio that includes R. Other alternatives also work, such as building R from source. You can determine the R version from the `renv` lock file: 

```bash
cat renv.lock | grep -A1 "\[R\]" | sed -En "s/Version=(.*)$/\1/p"
```

3. Build and deploy the docker image. This image contains the environment for your production code. The image can then be used as the basis for containers to execute your code in a a tool like Kubernetes.
