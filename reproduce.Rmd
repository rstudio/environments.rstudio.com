---
title: "Reproduce"
description: |
  Reproduce Environments Over Time
---

This section covers a variety of strategies for reproducing R environments overtime. Selecting a strategy will depend on two factors: who is responsible for managing the environment and how open the environment is; e.g. can users install their own R packages? At first these two questions might seem similar, but separating the two uncovers common "danger zones" or "anti-strategies", which are areas on the map below that look like strategies but result in problems!




```{r mental-map}
library(ggplot2)
library(tibble)
library(ggrepel)
library(plotly)
mm <- tribble(
  ~x, ~y, ~label, ~Description, ~status,
  0.1, 0.1, 'Validated', 'Admins test and approve \n a subset of CRAN', TRUE,
  0.5, 0.5, 'Shared Baseline', 'All or most of CRAN, \n updated with R versions, \n tied to a system library', TRUE,
  0.5, 0.75, 'Wild West', 'Open access, \n not reproducible, \n how we learn', FALSE,
  0.8,0.8, 'Snapshot', 'Open access, user or system \n records per-project dependencies', TRUE,
  0.75, 0.2, 'Blocked', 'Backdoor package access, \n offline systems without a strategy', FALSE,
  0.2, 0.8, 'IT Tickets', 'Admins involved, \n no testing, \n slow updates, \n high risk of breakage', FALSE
)
bad1 <- tribble(
  ~x, ~y, ~label, ~Description, ~status,
  0, 0.2, NA, NA, NA,
  0, 1,NA, NA, NA,
  0.8, 1, NA, NA, NA
)

bad2 <- tribble(
  ~x, ~y, ~label, ~Description, ~status,
  0.2, 0, NA, NA, NA,
  1, 0,NA, NA, NA,
  1, 0.8, NA, NA, NA
)

good <- tribble(
  ~x, ~y, ~label, ~Description, ~status,
  0, -0.2,NA, NA, NA,
  1, 0.8, NA, NA, NA,
  0.8, 1, NA, NA, NA,
  0, 0.2, NA, NA, NA
)

good2 <- tribble(
  ~x, ~y, ~label, ~Description, ~status,
  0, 0,NA, NA, NA,
  0, 0.2, NA, NA, NA,
  1, 0.8, NA, NA, NA,
  0.2, 0, NA, NA, NA
)

p <- ggplot(mm, aes(x, y, label = Description)) + 
  geom_abline(slope = 1, intercept = 0.2, alpha = 0.2) + 
  geom_polygon(aes(x,y), fill = "red", data=bad1, alpha = 0.1) + 
  geom_polygon(aes(x,y), fill = "green", data=good, alpha = 0.1) + 
  geom_polygon(aes(x,y), fill = "green", data=good2, alpha = 0.1) + 
  geom_polygon(aes(x,y), fill = "red", data=bad2, alpha = 0.1) +
  geom_abline(slope = 1, intercept = -0.2, alpha = 0.2) + 
  geom_point(aes(x, y, color = status)) + 
  geom_text(aes(x, y, label = label), nudge_y = 0.025, nudge_x  = 0.025) + 
  scale_x_continuous(limits = c(0,1), breaks = seq(0,1,0.25), labels = c("Admins","", "", "", "Users")) +
  scale_y_continuous(limits = c(0,1), breaks = seq(0,1,0.25), labels =  c("Locked Down","", "", "", "Open")) +
  theme_minimal() + 
  scale_color_manual(breaks = NULL, values = c("#ff0000","#a3c586")) + 
  
  labs(
    x = "Who is Responsible for Reproducing the Environment?",
    y = "Package Access",
    color = NULL,
    title = "Reproducing Environments: Strategies and Danger Zones"
  ) # + 
  # theme(axis.line = element_line(linetype = 'solid', arrow = grid::arrow(length = grid::unit(1, units = 'mm'))))
pi <- ggplotly(p, tooltip = c('label'))
hide_legend(pi)
```

## Snapshot and Restore

renv

## Shared Baselines
Discusses how to use RSPM's "frozen" repos to recreate environments, including instructions for creating point-in-time libraries.

## Validated
Discusses how to pre-emptively "freeze" folks against a point in time (using RSPM) with a link to how Docker fits in.
docker with a fixed point in time


