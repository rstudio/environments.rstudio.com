...
- [Shared Baseline](./reproduce.html#shared-baseliness)  
- [Validated](./reproduce.html#validated)  
...
3. Prior to committing or pushing code changes, record the project's dependencies:

```
renv::snapshot()
# commit the resulting lock file, renv.lock, alongside your project code
```
...
baseline](./reproduce.html#shared-baselines) strategy. The main difference is the
...
- If you are working with a group of newer users, consider working with IT to setup the [shared baseline](./reproduce.html#shared-baselines) strategy. Be careful not to slip into the [ticket system](./reproduce.html#ticket-system) scenario, which occurs if you ask IT to restrict the system without teaching them how to manage shared baselines. It might make sense to use the shared baseline strategy by default, and allow experienced users to step into the snapshot strategy.  
...
- If your organization requires admin involvement for practical reasons, (e.g. you're working on offline server), consider adopting the [shared baseline](./reproduce.html#shared-baselines) strategy.

- If your organization requires admin involvement for strategic reasons (e.g. you have concerns about package licenses), consider adopting the [validation](./reproduce.html#validated.html) strategy.
