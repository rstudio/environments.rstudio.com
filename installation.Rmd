...
The best resource for understanding package installation is Wickham's ["What is a package?"](http://r-pkgs.had.co.nz/package.html#package).
 ...
 1. [`pak` package](https://github.com/r-lib/pak) is used to install packages interactively. `pak`  is designed to help humans install packages quickly and safely.
 2. [`remotes` packages](https://github.com/r-lib/remotes) is used to install packages programmatically. `remotes` is designed with to help machines install package. The `remotes` package is intentionally lightweight and does not, itself, have any dependencies.^[`remotes` powers `devtools::install_` and is a suitable, lightweight alternative.]