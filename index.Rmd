...
The file `churn-model/renv/2018-03-01/2019-03-01T09-30-22GMT` contains all the information you need to communicate your project's dependencies _at the moment you call `snapshot`_. The next time you call `snapshot`, a new file will be created with a new timestamp. The file `renv.lock` contains similar information, but will be over-written the next time you call `snapshot`.

If you are familiar with version control for your code, we recommend calling `snapshot` anytime you push or checkin changes to your code. Learn more about [recording environments with version control](./libraries.html#record-keeping). **In short, you should commit `renv.lock` alongside your code.**
