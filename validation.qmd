...
The first concern, reproducing environments, is covered at length by the different
[strategies](reproduce.qmd) for environment management. The [validated](validated.qmd) 
strategy is particularly useful for creating sets of approved packages, though
other strategies can be used depending on the context.
...
packages are accurate in their stated purpose. Unfortunately, with
`r nrow(available.packages(repo="https://cran.rstudio.com"))`^[Run on `r Sys.Date()`]
...
Not what you were expecting? Before continuing, here are some quick links to other
resources specific to validation in the clinical pharma space:  
...
<aside>
Many packages include a badge to quickly indicate their current CRAN status.
For example, this is the CRAN status badge for ggplot2:
...
In addition to documentation, a critical indicator that a package is ready for
prime time is checking to see whether the [package has tests](https://r-pkgs.had.co.nz/tests.html).
Normally, package authors include tests in a directory
[alongside their package code](https://github.com/tidyverse/ggplot2/tree/main/tests).
Tests help authors check their code for accuracy and prevent them from accidentally breaking code. 

Many packages will go a step further and report [test coverage](https://covr.r-lib.org/).
This metric indicates how much of the package code is currently tested.
Often package authors  will automatically run tests using a continuous integration
service and report test status and code coverage through public badges.

<aside>
The following badges show the current test coverage and status for ggplot2:

[![Travis Build Status](https://travis-ci.org/tidyverse/ggplot2.svg?branch=main)](https://travis-ci.org/tidyverse/ggplot2)
[![Coverage Status](https://img.shields.io/codecov/c/github/tidyverse/ggplot2/main.svg)](https://codecov.io/github/tidyverse/ggplot2?branch=main)
</aside>

## Documentation

A critical indicator of a package's health and usefulness is the level of documentation.
R packages provide documentation in a number of formats:  

-[Package READMEs](https://github.com/juliasilge/tidytext/blob/main/README.md)  
...
mirror](repositories.qmd#cran-mirrors) https://cran.rstudio.com. The easiest
...
```{r}
downloads <- cranlogs::cran_downloads("ggplot2", from = "2019-01-01", to = today() - ddays(2))
...
and the related [itdepends](https://github.com/r-lib/itdepends) tool. A few
quick tips:


- Package dependencies can be viewed in the package's Description file and come
in a [few flavors](http://r-pkgs.had.co.nz/description.html#dependencies):
`Suggests`, `Depends`, `Imports`, and `LinkingTo`.
- Package dependencies describe what a package relies on. For example ggplot2 imports
rlang, which means ggplot2 requires rlang in order to work. Reverse dependencies
indicate the opposite, so ggplot2 is a reverse dependency for rlang.
- You should understand how [package inter-dependencies impact reproducibility](reproduce.qmd#the-big-risk).    
- In addition to depending on other R packages, a package can have system requirements.
For example, the rJava package requires a Java installation. You can view system dependencies
for a package in the Description file, though a more complete listing is available
[here](https://sysreqs.r-hub.io/) or in [RStudio Package Manager](https://packagemanager.rstudio.com/client/#/repos/1/packages/curl).  
...
guidelines](https://github.com/tidyverse/ggplot2/blob/main/CONTRIBUTING.md).
...
Another indicator of a package's stability is the package's release history.
For packages on GitHub, this [release history](https://github.com/tidyverse/ggplot2/releases)
is often visible directly. You can also look for the package's
[NEWS](https://github.com/tidyverse/ggplot2/blob/main/NEWS) file.

Unfortunately, just looking at the number of releases or the date of the last
release does not paint the whole picture. Some packages will have lots of recent
releases because they are rapidly changing. Other packages might not have had a
release for quite some time - is this because the package has been abandoned?
Or is it because the package is really stable? Considering the package’s
[state of life](https://www.tidyverse.org/lifecycle/) can help answer these questions.
...
timeline <- tibble(
  x = c(ymd_hm("2007-05-01 12:00"), ymd("2019-01-01")),
  y = c(0.5,0.5)
)
...
repository](repositories.qmd#internal-repositories) to [track and audit
license usage](https://demo.rstudiopm.com/client/#/stats).

```{r}
ap <- available.packages(repos = c("CRAN" = "https://cran.rstudio.com"))
...
the question of package validation. Take a look at their [work](https://www.pharmar.org/overview/).
