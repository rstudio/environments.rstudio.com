...
- Line 7 now reference a depedency to `streamlit = "^1.8.1"`
- Line 12 references a development depedency to `black = "^22.3.0"`
...
*poetry.lock* was created after we installed *streamlit*. The lock file includes the depedencies we have declared via the CLI, **plus** all of the depedencies of those packages. The file can be large, so below we only show a small example of what the file looks like
...
This command will remove *black*, **and** it will also remove all of *black*'s' depedencies that we no longer need. Then, lets add *autopep8* as a depedency.

```bash
poetry add --dev autopep8
```

*pyproject.toml* has been updated. *black* is no longer a dev depedency and *autopep8* is.
...
In the [create a new project](#create-a-new-project) section we mentioned that *poetry* created a new virtual environment. Everytime we call `poetry add` or `poetry remove` we are modifying that virtual environment. In order to run a command **inside** the virtual environment we use the command `poetry run`. Lets take a look:
...
This section will dicuss how to publish your code to [RStudio Connect](https://www.rstudio.com/products/connect/) when you are using *poetry* for depedency management. One of the ways *RStudio Connect* disocveries an apps depdencies is by inspecing a projects *requirements.txt*. *RStudio Connect* does not know how to inspect *pyproject.toml* or *poetry.lock*.

Luckily for us there is a simple workaround. We can use the `poetry export` command to document all of the depdencies in a *requirements.txt* format.
...
*requirements.txt* includes all of the depdencies you specified via the command line, and all of their depdencies. Each package is also locked into a specific version.
...
Now you can publish to *RStudio Connect* as you normally would. First lets install *rsconnect-python* as a development depdency:
...
There are two key things your colleage will need:
...
3. Install the depdencies using `poetry install`.

That is it 🎉! Your colleageu will now be able to run the code using the `poetry run` command. They can also make changes to the environment with `poetry add` and `poetry remove`!
