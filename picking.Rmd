...
With `r nrow(available.packages(repo="https://cran.rstudio.com"))`^[Run on `r Sys.Date()`] R packages on CRAN, and more added each day, it can be difficult
for R users and administrators to determine which packages to use.
Unfortunately, it is impossible to provide a single list of "approved" packages.
Many packages are highly specific to a domain problem, a package suitable for
financial analysis wouldn't make sense for a biostatistician. Additionally, any
list would quickly become outdated.

<aside>
If you really just need a place to start, consider the workflows and packages described in:  
- [R for Data Science](https://r4ds.had.co.nz)  
- [Data Import, Wrangling, and Visualization](https://tidyverse.org)  
- [Models](https://github.com/tidymodels/tidymodels)  
</aside>

Instead, we recommend users learn a set of characteristics they can apply to
quickly judge a package's quality and stability. The great news about open
source software is the wide availability of data *about* packages, and the
option to inspect package source code!
...
In addition to documentation, a critical indicator that a package is ready for primetime is checking to see whether the [package has tests](http://r-pkgs.had.co.nz/tests.html). Normally, package authors include tests in a directory [alongside their package code](https://github.com/tidyverse/ggplot2/tree/master/tests). Tests help authors check their code for accuracy and prevent them from accidentally breaking code. 
...
The number of times a package is downloadeds can help you determine how frequently a package
...
- Package dependencies describe what a package relies on. For example ggplot2 imports rlang, which means ggplot2 requires rlang in order to work. Reverse dependencies indicate the opposite, ggplot is a reverse dependency for rlang.  
...
the question of [package validation](./validation.html). Take a look at their
[preliminary work](https://www.pharmar.org/packages/).
...
Julia Silge has written an excellent series of blogposts expanding on the topic
